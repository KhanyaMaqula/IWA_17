...
const createData = () => {           
  ...
  current.setDate(1);
  const startDay = current.getDay(); // this is to get the current date of the month to show in the calendar 
  const daysInMonth = getDaysInMonth(current);
  const weeks = createArray(5); // To get all the weeks to show under the week column
